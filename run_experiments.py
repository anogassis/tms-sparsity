...
def setup_single_experiment(
    ...
    """Setup a single experiment - separated from training loop."""
    torch.manual_seed(seed)

    model = ToyAutoencoder(m, n, final_bias=True)
    init_weights = generate_init_param(
        n, m, init_kgon, prior_std=prior_std,
        no_bias=no_bias, init_zerobias=init_zerobias, seed=seed
    )
    ...
    return model, optimizer, dataloader, dataloader_test, dataset, dataset_test


def train_multiple_models_parallel(
